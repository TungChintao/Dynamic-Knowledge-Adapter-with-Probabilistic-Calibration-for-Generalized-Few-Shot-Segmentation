...
from model.pspnet_aux import get_model_

from util import dataset_oem
from util import transform, transform_tri, config
from util.dice_ce_loss import CombinedLoss
...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/oem.yaml',
...
def get_model(args):
    ...
    base_params = [v for k,v in model.named_parameters() if 'aux' not in k]
    ...
    #criterion = CombinedLoss(ignore_index=args.ignore_label)
...
def main():
    ...
    train_transform = transform.Compose([
        ...
        transform.RandRotate90(args.rotate_list, padding=mean, ignore_label=args.padding_label),
    ...
    if args.data_set == 'pascal' or args.data_set == 'coco' or args.data_set == 'oem':
        train_data = dataset_oem.BaseData(mode='train', data_root=args.data_root,
                                          data_list=args.train_list,
                                          data_set=args.data_set, transform=train_transform, main_process=main_process(),
    ...
    if args.evaluate:
        if args.resized_val:
            ...
            val_transform = transform.Compose([
                ...
                transform.Normalize(mean=mean, std=std)])
        if args.data_set == 'pascal' or args.data_set == 'coco' or args.data_set == 'oem':
            val_data = dataset_oem.BaseData(mode='val', data_root=args.val_root, data_list=args.val_list,
                                            data_set=args.data_set, transform=val_transform, main_process=main_process(),
...
def train(train_loader, model, optimizer, criterion, epoch):
    ...
    #print("1111111111",len(train_loader))
    for i, (img, target) in enumerate(train_loader):
        ...
        output = model(img)
        #print(np.unique(output.cpu()).tolist())
...
def validate(val_loader, model):
    ...
    for i, logits in enumerate(val_loader):
        ...
        #pred = np.array(output.squeeze().cpu(), np.uint8)
        
        #cv2.imwrite('./result/preds/'+str(i) + '.png', pred)
