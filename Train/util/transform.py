...
mean = 1
std = 2
# 假设args.train_h和args.train_w已经被定义
transform = transforms.Compose([
    # 替换为FiveCrop或TenCrop
    transforms.FiveCrop(size=(h, w)),  # 或使用 TenCrop
    # 转换裁剪的图像为张量，并进行标准化处理
    # 注意: 因为FiveCrop和TenCrop返回的是元组，所以需要使用Lambda来处理这些图像
    transforms.Lambda(lambda crops: torch.stack([transforms.Normalize(mean=mean, std=std)(transforms.ToTensor()(crop)) for crop in crops]))
])
...
class RandRotate90(object):
    ...
    def __call__(self, image, label):
        if random.random() < self.p:
            angle = angle = random.choice(self.rotate)
            h, w = label.shape
...
class ColorJitter(object):
    def __init__(self, brightness=1.8, brightness_max_v=0.1, contrast=0.9, contrast_max_v=0.05, sharpness=0.9,
                 sharpness_max_v=0.05, autocontrast=True):
        self.brightness = brightness
        self.brightness_max_v = brightness_max_v
        self.contrast = contrast
        self.contrast_max_v = contrast_max_v
        self.sharpness = sharpness
        self.sharpness_max_v = sharpness_max_v
        self.autocontrast = autocontrast

    def __call__(self, image, label):
        # Brightness
        if random.random() < 0.5:
            v = _float_parameter(random.uniform(0, PARAMETER_MAX), self.brightness_max_v) + self.brightness
            image = Image.fromarray(np.uint8(image)).convert('RGB')
            image = ImageEnhance.Brightness(image).enhance(v)

        # Contrast
        if random.random() < 0.5:
            v = _float_parameter(random.uniform(0, PARAMETER_MAX), self.contrast_max_v) + self.contrast
            image = Image.fromarray(np.uint8(image)).convert('RGB')
            image = ImageEnhance.Contrast(image).enhance(v)

        # Sharpness
        if random.random() < 0.5:
            v = _float_parameter(random.uniform(0, PARAMETER_MAX), self.sharpness_max_v) + self.sharpness
            image = Image.fromarray(np.uint8(image)).convert('RGB')
            image = ImageEnhance.Sharpness(image).enhance(v)

        # AutoContrast
        if self.autocontrast and random.random() < 0.5:
            image = Image.fromarray(np.uint8(image)).convert('RGB')
