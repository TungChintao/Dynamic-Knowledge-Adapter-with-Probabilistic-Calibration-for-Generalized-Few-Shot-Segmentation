...
def adjust_background_to_highest_class(args, final_pred):

    background_prob = final_pred[:, :, 0, :, :]
    max_probs, max_indices = torch.max(final_pred, dim=2)
    is_background_highest_and_low = (max_indices == 0) & (background_prob < args.threshold_alpha)

    for i in range(0,8):
        final_pred[:, :, i, :, :][is_background_highest_and_low] = args.adjust_p
    return final_pred
