...
class Classifier(object):
    def __init__(self, args, base_weight, base_bias, e_weight, e_bias, m_weight, m_bias, d_weight, d_bias, n_tasks):
        ...
        self.snapshot_weight = base_weight.squeeze(0).squeeze(
            0).clone()  # Snapshot of the model right after training, frozen
        ...
        if base_bias is not None:
            ...
            self.snapshot_bias = torch.zeros((self.num_base_classes_and_bg,), device=base_weight.device).unsqueeze(
                0).repeat(self.n_tasks, 1)
        self.base_bias = torch.zeros((self.num_base_classes_and_bg,), device=base_weight.device).unsqueeze(0).repeat(
            self.n_tasks, 1)
        ...
        self.aux_e_weight = e_weight.clone()
        self.aux_e_bias = e_bias.clone()
        self.aux_m_weight = m_weight.clone()
        self.aux_m_bias = m_bias.clone()
        self.aux_d_weight = d_weight.clone()
        self.aux_d_bias = d_bias.clone()

        self.fine_tune_base_classifier = args.fine_tune_base_classifier
        self.lr = args.cls_lr
        self.aux_lr = args.aux_optim_lr
        ...
        self.threshold = args.threshold_beta
        self.temperature = args.temperature
        self.adjust_p = args.adjust_p
    ...
    def init_novel_cls(self, novel_weight, novel_bias):
        self.novel_weight = novel_weight.squeeze(0).repeat(self.n_tasks, 1, 1)
        self.novel_bias = novel_bias.unsqueeze(0).repeat(self.n_tasks, 1)

    def init_novel(self, novel_weight, novel_bias):
        self.novel_weight = novel_weight
        self.novel_bias = novel_bias
        # print(self.snapshot_weight.shape, self.base_weight.shape, self.base_bias.shape)
        # print(self.novel_weight.shape, self.novel_bias.shape)
    ...
    def get_logits(self, features: torch.tensor) -> torch.tensor:
        ...
        """
        features_e = F.conv2d(features.squeeze(0), self.aux_e_weight, bias=self.aux_e_bias)
        features_m = F.conv2d(features_e, self.aux_m_weight, bias=self.aux_m_bias)
        features_d = F.conv2d(features_m, self.aux_d_weight, bias=self.aux_d_bias)
        features = features + features_d.unsqueeze(0)
    ...
    def get_probas(self, logits: torch.tensor, bg=False) -> torch.tensor:
        """
        inputs:
            logits : shape [batch_size_val, num_novel_classes * shot or 1, num_classes, h, w]
        ...
        """
        probas = torch.softmax(logits / self.temperature , dim=2)
        if not bg:
            return probas

        background_prob = probas[:, :, 0, :, :]
        max_probs, max_indices = torch.max(probas, dim=2)
        updated_probas = probas.clone()
        # mask = (max_indices == 0) #& (background_prob < self.threshold)
        mask = background_prob < self.threshold
        updated_probas = torch.where(mask, torch.full_like(probas, self.adjust_p), probas)
        return updated_probas
    ...
    def self_estimate_pi(self, features_q: torch.tensor, unsqueezed_valid_pixels_q: torch.tensor) -> torch.tensor:
        ...
        probas = self.get_probas(logits_q, True).detach()
    ...
    def optimize(self, features_s: torch.tensor, features_q: torch.tensor, gt_s: torch.tensor) -> torch.tensor:
        ...
        for m in params:
            m.requires_grad_()
        aux_params = [self.aux_e_weight, self.aux_e_bias, self.aux_m_weight, self.aux_m_bias, self.aux_d_weight, self.aux_d_bias]
        for p in aux_params:
            p.requires_grad_()

        optimizer = torch.optim.SGD([{"params": aux_params, "lr": self.aux_lr},
        ...
        for iteration in range(self.adapt_iter):
            ...
            proba_s, proba_q = self.get_probas(logits_s), self.get_probas(logits_q, True)
